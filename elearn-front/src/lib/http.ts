...
$api.interceptors.request.use((config: InternalAxiosRequestConfig) => {
  ...
  // Але основний механізм - це Cookie
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && error.config && !originalRequest._retry) {
      ...
      try {
        ...
        // КРИТИЧНЕ ВИПРАВЛЕННЯ REACT ERROR:
        // Очищаємо дані
        localStorage.removeItem('user_data')
        localStorage.removeItem('access_token')
        
        // Даємо React 100мс, щоб завершити поточні оновлення DOM перед редіректом
        // Це виправить помилку "NotFoundError: Failed to execute insertBefore"
        setTimeout(() => {
...
export default $api
/**
 * Extracts meaningful error message from API error response
 */
